...
def main_backup(args):
	...
	print "\n\tDownloaded %d | Folders %d | Skipped %d | Refreshed %d\n" % (runtime['downloaded'], runtime['folders'], runtime['skipped'], data['refresh'])
...
def main():
	...
	parser_meta.add_argument('-b', '--blank-members', action='store_true', default=False, help = 'Only uses the supplied --members list')
	...
	parser_backup = subparsers.add_parser('backup', description = 'Handle automated backup of remote S3 files', help = 'Backup files from S3')
	# parser_backup.add_argument('--aws-config', type = file, dest = 'aws_config', help = 'Manually set the AWS configuration file')
	parser_backup.add_argument('--aws-config', type = file, dest = 'aws_config', help = 'Manually set the AWS configuration file')
	parser_backup.add_argument('--meta-config', type = file, dest = 'meta_config', help = 'Manually set the meta-data configuration file')
